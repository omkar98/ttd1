...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning("response received: "+str(response.json()))
    except:
        logger.critical("ERROR from TTD")
    if(not response.json().get('status') == 'fail'):
        avl = (response.json().get('result').get('20230723').get('avl'))
    if (avl==0): 
      avl = (response.json().get('result').get('20230724').get('avl'))
    message = 'Subject: {}\n\n{}'.format(json.dumps(response.json().get('result').get('20230723')), "Available for 23/24 July")
    if(avl>0):
